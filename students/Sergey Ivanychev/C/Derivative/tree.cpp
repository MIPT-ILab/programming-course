/**
		...
		@par			Changelog V 1.01
						-- Added function optimization (sin(5), log(6, 2.7))
...
/**
		...
		@return									TREE_INVALID_INPUT - invalid input
												TREE	_ARG_TREE_NULL - argumented tree pointer is NULL
...
/**
			tree_burn							functions saves inputted tree type and it's value to the argumented tree

			@param			my_tree				tree to burn
			@param			type				saving tree type
...
/**
			head_destr							function destroys tree head and the whole tree depending on second parameter.
												If the parameter is TREE_DESTR_HEAD, function is waiting for the tree** parameter
												to save the root. Otherwise, the whole tree will be destroyed
			
			@param			my_head				argumented head
			@param			cond				deleting parameter

			@return								TREE_OK or what tree_destr() returns
...
/**
			...
			tree_delete								function deletes the node and left or right tree, depending on parameter. Another tree
													if it exists, will be sticked to the papa of current tree

			@param[out]			my_tree				deleting tree
			@param				side				the side that will be saved
...
/**
			tree_save_int							function saves int value to the tree

			@param[out]			my_tree				argumented tree
...
/**
			tree_save_double						function saves double value to the tree
...
/**
			tree_save_string						function that saves string to the tree

			@param[out]			my_tree				argumented tree
			@param[out]			word				saving string
...
/**
			tree_init									function creates the node, burns it with type and head pointer and sticks to
														the argumented tree to the argumented side

			@param[out]			dest_save_tree_token	where the pointer to created tree will be saved
			@param[out]			head_ptr				head pointer that will be save to create tree
			@param[out]			tree_to_stick_to		you know:)
			@param[out]			func_name				name of function that calls it. Necessary for debug printing
			@param				tree_type				saving tree type
			@param				add_side				sticking side

			@return										SNTX_CTR_FAILED
														SNTX_BURN_FAILED
														SNTX_ADD_FAILED
														TREE_OK
...
/**
			tree_copy									creates a copy of current tree

			@param[out]			copied_tree				tree to copy
			@param[out]			copy					pointer to head pointer, where the new tree will be saved
...
/**
			tree_optimize								function that proceeds some optimizations to the tree
														calculates system "num-sign-num" and examples like 0 / var, var * 0

			@param[out]			my_tree					tree to optimize
			@param[out]			is_optimized			where true or false will be save, verifying were the tree optimized or not
...
/**
			tree_save_head								updates the head pointer in each node of the tree

			@param[out]			my_tree					updating tree pointer
			@param[out]			head					updating head pointer
**/
...
int node_insert(tree* new_papa, tree** save_to, int side_papa, int side_child)
{
	VERIFY(new_papa == NULL,				TREE_BAD,									"TREE INSERT: Argumented tree pointer is NULL");
	VERIFY(save_to  == NULL,				TREE_BAD,									"TREE INSERT: Argumented tree save-to is NULL");
	VERIFY(side_papa != LEFT && side_papa != RIGHT,	TREE_BAD,							"TREE INSERT: Argumented side_papa is not LEFT or RIGHT");
	VERIFY(side_child != LEFT && side_child != RIGHT && side_child != NONE, TREE_BAD,	"TREE INSERT: Argumented side_child is not LEFT or RIGHT");
	...
	*save_to = new_node;
	VERIFY1(ret != TREE_OK,					ret,			"NODE INSERT: Tree construction failed, error [%d]", ret);
	...
	if (past_child == NULL) return TREE_OK;
...
bool is_num_leaf(tree* my_tree) //! That fucnction checks whether node is numeric leaf with no children
{
	if (my_tree != NULL && my_tree -> left == NULL && my_tree -> right == NULL && my_tree -> type == TR_N) return true;
	return false;
};
...
{
	assert(my_tree);
	if (my_tree -> type == TR_N && IS_ZERO(*(double*)(my_tree -> value) - num)) return true;
	return false;
}
...
void clear_leafs(tree* my_tree)
{
	assert(my_tree != NULL);
	int ret = TREE_OK;
	if (my_tree -> left != NULL)  ret = tree_destr(my_tree -> left);
	my_tree -> left = NULL;
	assert(ret == TREE_OK);
	if (my_tree -> right != NULL) ret = tree_destr(my_tree -> right);
	my_tree -> right = NULL;
	assert(ret == TREE_OK);
...
bool calc_if_possible(tree* my_tree)
{
	assert(my_tree);
	double res = 0;
	if (is_num_leaf(my_tree -> left) && is_num_leaf(my_tree -> right) && my_tree -> type == TR_SIGN)
	{
		
		calc_operand(*(double*)(my_tree -> left -> value), *(double*)(my_tree -> right -> value), *(int*)(my_tree -> value), &res);
		
		clear_leafs(my_tree);

		DBG_FREE fprintf(stdout, "[%08x] tree.cpp, calc_if_possible, my_tree -> value\n", my_tree -> value);
		free(my_tree -> value);
		my_tree -> value = NULL;
...
{
	VERIFY(my_tree == NULL,		TREE_ARG_TREE_NULL, "TREE CLEAR: Argumented tree pointer is NULL");
...
int count_args(tree* my_tree, int* argc)
{
	...
	{
		if (temp_tree -> type != TR_F) assert(temp_tree -> left);
		assert(temp_tree -> right);
		temp_tree = temp_tree -> right;
		i++;
	}
	VERIFY1((i == MAXVARS && temp_tree -> type == TR_NONE),			TREE_BAD,		"FUNC COUNT ARGS: Too many arguments for function [%08x]", my_tree);
	*argc = i;
...
bool is_possible_func_optimize(tree* my_tree, const int argc)
{
	...
	for (int i = 1; i < argc; ++i)
	{
		assert(temp_tree -> left);
		assert(temp_tree -> right);
		if (temp_tree -> left -> type != TR_N) return false;
		temp_tree = temp_tree -> right;
	}
	if (temp_tree -> type != TR_N) return false;
	return true;
}

#define CALL(func) if ((ret = func) != TREE_OK) return ret;
...
int tree_optimize(tree* my_tree, bool* is_optimized)
{
	...
	if (my_tree -> type == TR_SIGN && *(int*)(my_tree -> value) == '/' && my_tree -> right != NULL && my_tree -> right -> type == TR_N && IS_ZERO(*(double*)(my_tree -> right -> value)))
			return SNTX_DIV_BY_ZERO;
	...
	if (!(my_tree -> left == NULL || my_tree -> right == NULL))
	{
		...
		if (calc_if_possible(my_tree))  *is_optimized = true;
		if (calc_zero_operand(my_tree)) *is_optimized = true;
		if (calc_operand_one(my_tree))  *is_optimized = true;
		if (calc_sum_zero(my_tree))     *is_optimized = true;
	}
	else
	{
	//!				Optimizing functions, switched off by default
	//ret = func_optimize(my_tree, &cur_optimization);
	//VERIFY(ret != TREE_OK,		ret,	"TREE OPTIMIZE: Func optimize failed");
	//if (cur_optimization) *is_optimized = true;
	}
	return SNTX_OK;
...
int head_ctr(tree_head** my_tree)
{
	...
	VERIFY1(ret != TREE_OK, TREE_CTR_FAILED,			"# HEAD CTR ERROR: TREE_CTR() failed to construct tree, error [%d]\n", ret);
...
#define DRAW_BRANCH_(side) 	fprintf(strout, "%s|\n", space); fprintf(strout, "%s" #side "---", space);
...
void tree_dump_in_order(FILE* strout, tree* my_tree, char* space)
{
	...
	else tree_dump_in_order(strout, my_tree -> left, next_space);
		
	DRAW_BRANCH_(R);
	sprintf(next_space,"%s    ", space);	
	if (my_tree -> right == NULL) fprintf(strout, "%s\n", nil);
	else tree_dump_in_order(strout, my_tree -> right, next_space);
...
#define RETURN_BAD(error, msg) {*size = 0; ERR(msg); return error;}
#define RETURN_BAD1(error, msg, arg1) {*size = 0; VERIFY1(true, error, msg, arg1);}
...
void tree_save_head(tree* my_tree, tree_head* head)
{
	...
	if (my_tree -> left  != NULL) tree_save_head(my_tree -> left , head);
	if (my_tree -> right != NULL) tree_save_head(my_tree -> right, head);
}
...
int tree_add_left(tree* my_tree, tree* adding)
{
	...
	int tr_counter = 0;
	VERIFY1(tree_check(my_tree, &tr_counter) == TREE_CHECK_BAD, TREE_INJURED,	"# ADD LEFT ERROR: [%08x] element, tree is broken\n", my_tree);
	tr_counter = 0;
...
int tree_destr(tree* my_tree)
{
	...
	if (my_tree -> papa != NULL)
	{
		if (my_tree -> papa -> left  == my_tree) my_tree -> papa -> left  = NULL;
		if (my_tree -> papa -> right == my_tree) my_tree -> papa -> right = NULL;
	}
...
void tree_save_int(tree* my_tree, int value)
{
	...
	{
		free(my_tree -> value);
		DBG_FREE fprintf(stdout, "[%08x] tree.cpp, tree_save_int, my_tree -> value\n", my_tree -> value);
		my_tree -> value = NULL;
	}
...
void tree_save_double(tree* my_tree, double value)
{
	...
	{
		free(my_tree -> value);
		DBG_FREE fprintf(stdout, "[%08x] tree.cpp, tree_save_double, my_tree -> value\n", my_tree -> value);
		my_tree -> value = NULL;
	}
	my_tree -> value = calloc(1, sizeof(double));
	assert(my_tree -> value);
...
void tree_save_string(tree* my_tree, char* word)
{
	...
	{
		free(my_tree -> value);
		DBG_FREE fprintf(stdout, "[%08x] tree.cpp, tree_save_string, my_tree -> value\n", my_tree -> value);
		my_tree -> value = NULL;
	}
