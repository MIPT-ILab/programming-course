...
int tex_at_deal(FILE* tex_output, FILE* tex_template, tree* func, tree* opt_func, tree* der, tree* opt_der, char* var);


#define CALL(func)			ret = func; if (ret != CPLR_OK) return ret;
#define TEX_PRINT(tree)		if (strcmp(word, #tree) == 0) {CALL(tex_print(tree, tex_output)); return CPLR_OK; }
...
int tex_build(FILE* tex_output, FILE* tex_template, tree* func, tree* opt_func, tree* der, tree* opt_der, char* var)
{
	...
	VERIFY(opt_func == NULL,	CPLR_BAD,	"TEX EXPORT: Argumented optimized func pointer is NULL");
	VERIFY(der == NULL,			CPLR_BAD,	"TEX EXPORT: Argumented derivative pointer is NULL");
	VERIFY(opt_der == NULL,		CPLR_BAD,	"TEX EXPORT: Argumented optimized derivative pointer is NULL");
	
	VERIFY(tex_output == NULL,	CPLR_BAD,	"TEX EXPORT: Argumented output stream is NULL");
	VERIFY(tex_template == NULL,CPLR_BAD,	"TEX EXPORT: Argumented tex template pointer is NULL");
	...
	while ((c = fgetc(tex_template)) != EOF)
	{
		...
		CALL(tex_at_deal(tex_output, tex_template, func, opt_func, der, opt_der, var));
	}
...
{
	char word[MAXLINE] = {};
	int ret = fscanf(tex_template, "%[A-Za-z_]", word);
...
int tex_print(tree* my_tree, FILE* tex_output)
{
	VERIFY(my_tree == NULL, CPLR_BAD,	"TEX PRINT: Argumented tree pointer is NULL");
	assert(tex_output);
	int ret = 0;
...
int tex_sign(tree* my_tree, FILE* tex_output)
{
	...
	VERIFY1(INT(my_tree) != '+' &&
			INT(my_tree) != '-' &&
			INT(my_tree) != '*' &&
			INT(my_tree) != '/',			CPLR_BAD,	"TEX SIGN: Invalid sign, got [%c]", INT(my_tree));
...
int tex_add_sub(tree* my_tree, FILE* tex_output)
{
	...
	int ret = 0;
	VERIFY(my_tree -> left == NULL || my_tree -> right == NULL,		CPLR_BAD,	"TEX ADD SUB: Invalid sum node");


	bool no_brackets = false;
	if (my_tree -> papa != NULL && is_papa_add_sub(my_tree) == true) 
		no_brackets = true;
	if (my_tree -> papa != NULL && TYPE(my_tree -> papa) == TR_SIGN && INT(my_tree -> papa) == '/')
		no_brackets = true;
	if (my_tree -> papa -> type == TR_PGM || my_tree -> papa -> type == TR_F) no_brackets = true;
...
int tex_mul(tree* my_tree, FILE* tex_output)
{
	...
	assert(INT(my_tree) == '*');
	int ret = 0;
	VERIFY(my_tree -> left == NULL || my_tree -> right == NULL,		CPLR_BAD,	"TEX ADD SUB: Invalid mul node");
	...
	if (put_dot == false && my_tree -> left -> right && my_tree -> right -> left && TYPE(my_tree -> right -> left) == TR_N && TYPE(my_tree -> left -> right) == TR_N)
	{
		PUT(" \\cdot ");
		put_dot = true;
	}
...
int tex_div(tree* my_tree, FILE* tex_output)
{
	...
	assert(INT(my_tree) == '/');
	int ret = 0;
	VERIFY(my_tree -> left == NULL || my_tree -> right == NULL,		CPLR_BAD,	"TEX ADD SUB: Invalid div node");
...
int tex_f(tree* my_tree, FILE* tex_output)
{
	...
	{ CALL(tex_pow(my_tree, tex_output)); return CPLR_OK;}

	if (FUNC_IS("log"))
	{ CALL(tex_log(my_tree, tex_output)); return CPLR_OK;}
	...
	if (FUNC_IS("ln"))
	{
		...
		bool is_sign = false;
		if (TYPE(my_tree -> right) == TR_SIGN) is_sign = true;
...
#define IS_ONE(ch_tree) (TYPE(ch_tree) == TR_N && fabs(DOUBLE(ch_tree) - 1.0) < DBL_EPSILON)
int tex_pow(tree* my_tree, FILE* tex_output)
{
	assert(TYPE(my_tree) == TR_F);
	...
	if (strcmp(WORD(my_tree), "exp") == 0) is_e = true;

	if (!(is_e))
	{
		if (TYPE(my_tree -> right -> left) == TR_SIGN) PUTC('(');
...
int tex_log(tree* my_tree, FILE* tex_output)
{
	...
	bool is_sign = false;
	if (TYPE(my_tree -> right -> right) == TR_SIGN) is_sign = true;
