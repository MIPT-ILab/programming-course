...
#define ARR_ASSERT(i, left, right)																								\
	if ( !(((left) <= (i))&&((i) <= (right))) )																					\
	{																															\
		printf("Program %s crashed, Array index is out of range, Line %d", __FILE__, __LINE__);									\
		abort();																													\
	}
...
/**
		stack			type defines stack itself, which works in "First in, first one" way
		
		@par			(WIKIPEDIA) In computer science, a stack is a particular kind of abstract data type\
						or collection in which the principal (or only) operations on the collection are the addition of an entity to the collection, 
...
/**
		cpu				this struct includes stack and (for now) 4 registers
**/
...
int plot(char* input_stream, char* output_stream, char* error_stream, cpu* my_cpu, double x_begin, double x_end, double dx, double ymin, double ymax, int display_width, int display_height);
