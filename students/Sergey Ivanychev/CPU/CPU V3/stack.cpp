...
int stack_print(stack* user_stack)
{
	...
	for (int i = user_stack -> size - 1; i >= 0; --i) 
	{
		...
		if (fprintf(stdout, "%lg", user_stack -> data[i]) < 0) return ERR_PRINT;
		putchar('\n');
	}
		return OK;
}
...
int stack_dump(FILE* stream, stack* user_stack)
{
	fprintf(stream, "\nI'm stack\n");
	if (!(user_stack)) 	return ERR_NULL;
	fprintf(stream,"stack[%08x] %s\n"
			"Maxsize = %d\n"
			"Size = %d\n", user_stack, stack_ok(user_stack) ? "BAD" : "OK!", user_stack -> maxsize, user_stack -> size);
